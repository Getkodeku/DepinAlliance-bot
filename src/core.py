...
class Depin:
    ...
    def user_data(self, user_id: str):
        ...
        try:
            ...
            log(
                green
                + f"Username: {white}{user_info.get('username', 'N/A')} {green}| Status: {white}{user_info.get('status', 'N/A')}"
            )
            log(
                green
                + f"Points: {white}{user_info.get('point', 0):,.0f} {green}| Mining Power: {white}{user_info.get('miningPower', 0):,.0f}"
            )
            log(
                green
                + f"Level: {white}{user_info.get('level', 0):,.0f} {green}| Experience: {white}{user_info.get('xp', 0):,.0f}"
            )
            log(
                green
                + f"Skill point: {white}{user_info.get('pointSkill', 0):,.0f} {green}| Total device: {white}{user_info.get('totalDevice', 0):,.0f}"
            )
            ...
            log(
                black
                + f"AttributeError: {red}{e}. Possible NoneType encountered during response parsing."
            )
        ...
        for day in checkin_data:
            if day["isChecked"]:
                ...
                if checkin_response.get("status") == "success":
                    log(
                        green
                        + f"Daily check-in successful! Points received: {white}{checkin_response.get('data', 0)}"
                    )
                    checked_in_days += 1
                else:
    ...
    def claim_mining(self, user_id: str):
        ...
        if claim_data is not None:
            if claim_data.get("point") <= 1:
                ...
                log(
                    green
                    + f"Claimed: {white}{claim_data.get('point', 0):,.0f} {green}points | Bonus: {white}{claim_data.get('bonusReward', 0)}"
                )
        return
        ...
        for quest in partner_quests:
            for mission in quest.get("missions", []):
                if mission.get("status") is None:
                    self.handle_task(user_id, mission["id"], "verify", mission["name"])
                    self.handle_task(user_id, mission["id"], "claim", mission["name"])
        log(blue + f"Other task may need a verifications!")
    ...
    def handle_task(self, user_id: str, task_id: str, action: str, task_name: str):
        ...
        if success:
            log(green + f"Succeeded {white}{task_name}")
    ...
    def get_skills(self, user_id: str):
        ...
        headers = {**self.base_headers, "Authorization": f"Bearer {token}"}
        return (
            self._request("GET", "/users/skills", headers=headers).get("data", {}).get("skill", [])
        )
    ...
    def upgrade_skill(self, user_id: str):
        ...
        response = requests.post(
            f"{self.base_url}/users/upgrade-skill", headers=headers, json=payload
        )
        if response.status_code == 200:
            response_data = response.json()
            if (
                response_data.get("status") == "error"
                and response_data.get("message") == "MSG_USER_SKILL_ANOTHER_WAITING_UPGRADE"
            ):
                log(
                    yellow
                    + f"Can't upgrade {white}{skill_name}! {yellow}another skill is on upgrade."
                )
            else:
                log(green + f"Successfully upgraded {white}{skill_name}.")
                skills = self.get_skills(user_id)
                waiting_time = next(
                    (skill["timeWaiting"] for skill in skills if skill["skillId"] == skill_id), 0
                )
            ...
            log(
                red
                + f"Failed to upgrade {white}{skill_name}. {red}Response: {black}{response.json()}"
            )

    def open_box(self, user_id: str, max_price: str):
        token = self.local_token(user_id)
        if not token:
            log(red + "Error: Token not found.")
            return
        ...
        while True:
            estimate = self._request(
                "POST", "/devices/estimate-use-key", headers=headers, json=payload
            )
            if estimate.get("status") != "success":
                log(black + "Error estimating use key: ", estimate.get("message"))
                break

            points_needed = estimate.get("data", 0)
            if points_needed >= max_price:
                log(yellow + "Max price exceeded, box will not be opened.")
                break
            ...
            if message == "MSG_ITEM_OPEN_NOT_ENOUGH":
                log(yellow + "You have no box to open!")
                break
                ...
                for reward in use_key.get("data", []):
                    ...
                    log(
                        green
                        + f"Type: {white}{reward_type}{green}, Name: {white}{reward_name}{green}, Point: {white}{reward_point}"
                    )
            else:
    ...
    def get_items_by_type(self, user_id: str, item_type: str):
        ...
        response = self._request(
            "GET", f"/devices/user-device-item?type={item_type}&page=1&size=12", headers=headers
        )
        ...
        if response.get("status") == "success":
            ...
            if items:
                ...
                if current_item and current_item["miningPower"] < highest_power_item["miningPower"]:
                    log(
                        green
                        + f"Current {white}{item_type}{green} item has lower power. Unequipping: {white}{current_item['name']}{green} | Power: {white}{current_item['miningPower']}"
                    )
                    self.unequip_item(user_id, current_item["id"])

                log(green + f"Adding highest {white}{item_type}:")
                log(
                    green
                    + f"Name {white}{highest_power_item['name']} {green}| Power: {white}{highest_power_item['miningPower']:,.0f}"
                )
    ...
    def log_items(self, device_index, items):
        ...
        for group_name, group in grouped_items.items():
            if group:
                log(green + f"{f'{white} + {green}'.join(group)}")
    ...
    def add_item_to_device(self, user_id: str, item_id: int, item_type: str):
        ...
        for device_index in device_indices:
            headers = {**self.base_headers, "Authorization": f"Bearer {token}"}
            response = self._request(
                "GET", f"/devices/add-item/{device_index}/{item_id}", headers=headers
            )
            if response.get("status") == "success":
                log(
                    green
                    + f"Successfully added {white}{item_id} {green}to device {white}{device_index}"
                )
                return
            else:
                ...
                if message == "MSG_DEVICE_USER_CANNOT_ADD_MORE_ITEM":
                    log(
                        yellow
                        + f"Can't add more {white}{item_type} {yellow}to device {white}{device_index}"
                    )
                else:
    ...
    def get_equipped_items(self, user_id: str, device_index: int):
        ...
        headers = {**self.base_headers, "Authorization": f"Bearer {token}"}
        response = self._request(
            "GET", f"/devices/user-device-item?index={device_index}&page=1&size=12", headers=headers
        )
        if response is None:
            ...
            log(
                black + f"Error fetching equipped items:",
    ...
    def auto_buy_item(self, user_id: str, device_index: int, max_item_price: float):
        ...
        while True:
            response = self._request(
                "GET",
                f"/devices/item?page={page_number}&sortBy=price&sortAscending=true&size=12",
                headers=headers,
            )
            if response.get("status") != "success":
                ...
                if item["code"] not in equipped_powers and item["price"] <= max_item_price:
                    for equipped_code, equipped_power in equipped_powers.items():
                        if item["miningPower"] > equipped_power:
                            buy_response = self._request(
                                "POST",
                                "/devices/buy-item",
                                headers=headers,
                                json={"number": 1, "code": item["code"]},
                            )
                            if buy_response.get("status") == "success":
                                log(
                                    green
                                    + f"Successfully bought {white}{item['name']} {green}with price {white}{item['price']}."
                                )
                            elif (
                                buy_response.get("status") == "error"
                                and buy_response.get("message") == "MSG_USER_POINT_NOT_ENOUGH"
                            ):
                                log(
                                    yellow
                                    + f"Not enough point to buy the {white}{item['name']} {yellow}items!"
                                )
                                return
                            else:
                                log(
                                    black + f"Error buying {item['name']}:",
                                    buy_response.get("message"),
                                )
                            break
